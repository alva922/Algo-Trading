...
API_KEY = "YOUR API CODE"  #get access with 10% discount https://eodhd.com/pricing-special-10?via=aiva
...
df = spy[['open', 'high', 'low', 'close', 'volume']]
...
mpf.plot(
    ...
    volume=True,
    style='charles',
    title='SPY Candlestick Chart & Volume',
    ylabel='Price (USD)',
    xlabel='Date',
    ylabel_lower='Volume',figsize=(14, 6)
)

df_base = pd.DataFrame(index=spy.index)
df_base["price"] = spy["close"]
...
plt.title(
    "SPY Bayesian Optimized MAMA Strategy vs Buy & Hold"
)
