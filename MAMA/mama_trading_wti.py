#https://medium.com/@alexzap922/from-spy-underperformance-to-macro-alpha-across-bonds-commodities-and-crypto-a-a735cbba2465
...
def get_wti():
    ...
    df = ticker.history(period="max")  # IMPORTANT: full available history
...
# assume your df is already loaded and indexed by Date
# df columns: Open High Low Close Volume

df = df.copy()

# ensure datetime index is clean (important for mplfinance)
df.index = pd.to_datetime(df.index)
df = df.sort_index()

# optional: remove timezone if present (mplfinance sometimes requires this)
...
mpf.plot(
    ...
    volume=True,
    style="yahoo",
    title="WTI Candlestick Chart & Volume",
    ylabel="Price USD",
    xlabel="Date",
    ylabel_lower="Volume",
    figsize=(14, 6)
